# currently just renames albums with some variant of '(Explicit)' in their
# title and applies a SQL script
import dataset
import sys
    ...
    with dataset.connect("sqlite:///last-fm.db") as db:
        ...
        for album in valid:
            for alt in valid[album]:
                sql = "UPDATE %s SET album = '%s' WHERE album = '%s'" % (
                    username,
                    album,
                    alt,
                )
