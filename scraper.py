...
class Scraper:
    def __init__(self, username):
        ...
        self.RECENT_URL = (
            self.ROOT_URL + "/?method=user.getrecenttracks&user=" + username
        )
    ...
    # flattens the JSON into a uniform depth (i.e. a series of keys and
    # non-nested values)
    # modified from http://stackoverflow.com/a/6027615/254187 to strip
    # pound symbols
    ...
    def get_all_scrobbles(self):
        ...
        for page in reversed(range(1, TOTAL_PAGES + 1)):
            scrobbles = requests.get(self.RECENT_URL % (page, self.PER_PAGE)).json()[
                "recenttracks"
            ]["track"]
            sys.stdout.write("\r" + (" " * 60))
            sys.stdout.write(
                "\rRetrieving scrobble history...\t%d of %d\tfor %s."
                % (TOTAL_PAGES - page + 1, TOTAL_PAGES, self.USER)
            )
            sys.stdout.flush()
    ...
    def update_scrobbles(self):
        with dataset.connect("sqlite:///last-fm.db") as db:
            rts = db.query(
                "SELECT MAX(timestamp) as recent from %s" % self.USER
            ).next()["recent"]
        ...
        inserted = 0
        for page in reversed(range(1, TOTAL_PAGES + 1)):
            scrobbles = requests.get(UPDATE_URL % (page, self.PER_PAGE)).json()[
                "recenttracks"
            ]["track"]
            inserted += self.insert_scrobbles(reversed(scrobbles))
            sys.stdout.write("\r" + (" " * 60))
            sys.stdout.write(
                "\rUpdated scrobble history with %d new scrobble(s) for %s."
                % (inserted, self.USER)
            )
            sys.stdout.flush()
        if inserted != 0:
    ...
    def artists(self):
        ...
        with dataset.connect("sqlite:///last-fm.db") as db:
            ...
            for index, row in enumerate(result):
                artist = requests.get(self.ARTIST_URL % row["artist"]).json()
                sys.stdout.write(
                    "\rRetrieving artist info...\t%s of %s"
                    % (str(index), str(totalArtists))
                )
                sys.stdout.flush()
                try:
            ...
            sys.stdout.write(
                "\rRetrieving artist info...\t{0} of {0}".format(str(totalArtists))
            )
            sys.stdout.flush()
...
if __name__ == "__main__":
    ...
    for username in usernames:
        ...
        with dataset.connect("sqlite:///last-fm.db") as db:
            sql = (
                "SELECT COUNT(name) as count FROM sqlite_master WHERE type='table' AND name='%s'"
                % username
            )
